...
conn = mariadb.connect(
	user="winters", password="root", host="localhost", database="ctf", autocommit=True
)
...
def login():
	...
	try:
		cursor.execute(
			# Here users is the table with the column data 
	...
	user, admin = row
	session["user"] = user
	session["admin"] = admin # Could be either 1 or 0, we want this to be 1 inorder get the flag
	return jsonify({}), 200
...
def register():
	...
	# Inorder to solve we need to bypass this check so data[admin] should return false but in the database it should be true
